...
def generate(generated_text, output_directory_path, supervisions, recordings, mean,std):
    sp2.create_cs_audio(generated_text, output_directory_path, supervisions, recordings, mean,std)
...
def main():
    ...
    supervisions, recordings, mean,std = sp2.load_dicts_modified(sup_path, rec_path)
    ...
    for i, s in enumerate(slice):
        p = multiprocessing.Process(target=generate, args=(s,outdir,supervisions, recordings, mean,std))
        p.start()
        processes.append(p)
